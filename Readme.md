...
## **`Clase Function_B`**
Se utiliza para representar las funciones. Cada instancia de la clase representa una única función y almacena información relevante del mismo, como su nombre(Name_function), los parametros(variable_param = param) y un Nodo que corresponde al cuerpo de la funcion(Operation_Node).
...
- Usar condicionales `if-else` asimismo como en casos anteriores, respetando su [ estructura ](#estructura-1)
